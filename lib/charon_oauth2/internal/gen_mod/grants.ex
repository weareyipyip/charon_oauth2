  ...
  def generate(schemas_and_contexts, repo) do
    quote location: :keep,
      ...
      ## Doctests
          ...
          iex> grant = insert_test_grant!(resource_owner_id: insert_test_user().id)
          ...
          iex> %{id: id, code: code} = insert_test_grant!(resource_owner_id: insert_test_user().id)
      ...
      ## Doctests
          ...
          iex> insert_test_grant!(resource_owner_id: insert_test_user().id)
      ...
      ## Examples / doctests
          ...
          iex> {:ok, _} = insert_test_grant(resource_owner_id: insert_test_user().id)
          ...
          iex> insert_test_grant(resource_owner_id: insert_test_user().id, authorization_id: -1) |> errors_on()
          %{authorization: ["does not exist"]}

          # resource owner must exist and must match the authorization's owner
          iex> user1 = insert_test_user().id
          iex> user2 = insert_test_user().id
          iex> insert_test_grant(resource_owner_id: user1, authorization_id: insert_test_authorization!(resource_owner_id: user2).id) |> errors_on()
          %{authorization_id: ["belongs to other resource owner"]}

          # type must be one of client grant_type's
          iex> client = insert_test_client!(owner_id: insert_test_user().id, grant_types: ~w(refresh_token))
          iex> authorization = insert_test_authorization!(resource_owner_id: insert_test_user().id, client_id: client.id)
          ...
          iex> insert_test_grant(resource_owner_id: insert_test_user().id, redirect_uri: "https://boom") |> errors_on()
          %{redirect_uri: ["does not match client"]}

          # redirect_uri is required if client has multiple uris set
          iex> client = insert_test_client!(owner_id: insert_test_user().id, redirect_uris: ~w(https://a https://b))
          iex> authorization = insert_test_authorization!(resource_owner_id: insert_test_user().id, client_id: client.id)
      ...
      ## Examples / doctests
          ...
          iex> valid = insert_test_grant!(resource_owner_id: insert_test_user().id)
          iex> expired = insert_test_grant!(resource_owner_id: insert_test_user().id)
